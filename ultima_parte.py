    ...
    def escalonamento_round_robin(cls):
        ...
        while processos_restantes:
            if not fila:
                ...
                tempo += 1
                for processo in processos_restantes:
                    if processo.techegada <= tempo and id(processo) not in adicionados:
                        fila.append(processo)
                        adicionados.add(id(processo))
                continue
            ...
            fatia = min(quantum, processo_atual.texec)
        ...
        total_espera = sum(
            tempos_de_conclusao[i] - cls.lista_de_processos[i].techegada - cls.lista_de_processos[i].tempo_execucao_original
            for i in tempos_de_conclusao
        )
        print(f'Tempo médio de espera: {total_espera / len(tempos_de_conclusao)}\n')
